...
def simulate_population(N0, PARAMS, T_FINAL, LANDSCAPE_LEN):
	...
	for t in range(0, T_FINAL):
		...
		N_A[t+1][0], N_A[t+1][1] = deterministic_movement(N_A[t+1][0], N_A[t+1][1], PARAMS) # does not generalize 
...
with basic_model:
	...
	for t in range(0, T_FINAL):
		for i in range(0, LANDSCAPE_LEN):
			...
			#print(PARAMS["alpha"])
			nextNB, nextNJ, nextNA = deterministic_pop_dynamics(N_B[t][i], N_J[t][i], N_A[t][i], PARAMS)
			prNB = Poisson('prNB', mu=nextNB)
			prNJ = Poisson('prNJ', mu=nextNJ)
			prNA = Poisson('prNA', mu=nextNA)
